...
module Calc_v7 = struct 
  ...
  = fun e env -> 
		match e with
		| IF (cond, the_branch, else_branch)(*e1, e2 ,e3*) -> 
			let b = eval_cond cond in
			if (b) then (cond, the_branch, else_branch) else (cond, the_branch, else_branch)
...
module Calc_v8 = struct 
  type exp = 
		| LETF of string * string *exp * exp (* let f(a) = e1 in e2 *)
  ...
  = fun e env -> 
		match e with
		| LETF (fid, pid ,body, e) ->
			let env' = (fid, Func(pid,body)) ::env
			in (calc e env')
		| CALL (fid, arg) ->
			(match (List.assoc fid env)) with
			| Num_ -> failwith"invaild"
			| Func (pid, body) -> 
				(*let pid = arg in body*)
